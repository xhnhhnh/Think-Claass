/**
 * Request-level authorization for the `collaboration` HTTP surface.
 *
 * The domain's 16 routes - the task tree, team quests and peer reviews - were the ones the matrix
 * describes as "整族无鉴权": the pre-migration controllers never injected `Req`, so no handler ever
 * read the request actor and any anonymous caller could read or write any class's tree, quests and
 * reviews. Every handler now resolves the caller from the kernel's verified request context first.
 *
 ...
 *
 * `RequestActor.studentId` is only a hint the host may fill in; ownership is resolved from
 * `classroom.public` by the service on every request, so a stale or absent hint cannot widen
 * anyone's reach.
 */
