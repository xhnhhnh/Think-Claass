/**
 * The website self-update surface: 3 routes under `/api/admin/system/update`, ported verbatim from
 * `api/modules/admin/admin.update.ts`.
 *
 * It never touches the database - it reads `logs/update-status.json`, tails `logs/update.log`, reads
 * `CURRENT_VERSION` and, on Linux, runs `update.sh` detached. So the migration changed only two
 * things: `requireActorRole`/`throwAdminError` come from this plugin instead of `api/utils/**`, and
 * `process.platform === 'linux'` still decides whether the feature is offered at all.
 *
 * ## Known debt: `child_process` is imported directly, not declared
 *
 * The SDK's `ctx.capabilities.childProcess` is `null` unless the manifest declares
 * `data.capabilities: ["child_process"]` - and the runtime **rejects** a manifest that declares it
 * without `isolation: "worker"` (`packages/plugin-runtime/src/host.ts`, `unsupportedDeclarations`),
 * while worker isolation does not exist until P6. So the three honest options today are: declare a
 * capability this plugin cannot actually have, claim `isolation: "worker"` while running in process,
 * or import `node:child_process` directly and say so. This is the third one, and it is recorded in
 * the manifest's `_known_debt`; P6 closes it when the worker isolation level lands.
 *
 * The alternative - refusing to spawn - would change product behaviour on Linux deployments, which
 * is exactly what a migration must not do.
 */
...
import { ok, requireActorRole } from './admin.support.js';
