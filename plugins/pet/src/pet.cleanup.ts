/**
 * pet's account-deletion cleanup rule.
 *
 ...
 *
 * One statement, ported verbatim from `admin.repository.ts` line 470:
 * `pets.deleteMany({ student_id: { in: studentIds } })`, one of the statements the cascade ran
 * inside `if (studentIds.length)` (lines 457-478). `inList` returns `null` for an empty student set,
 * which is that guard: an account with no students owns no pets, and `IN ()` must never widen into
 * "every row".
 */
...
export function createPetCleanupRule(): CleanupRule {
  return {
    tables: TABLES,
    ...
    run(tx: DbApi, subject): void {
      ...
      if (students) {
        tx.run(`DELETE FROM pets WHERE ${students.sql}`, students.params);
      }
    },
  };
}
