...
export class InsightsService {
  private getClassOwnedByTeacher(classId: number, teacherId: number) {
    return db.prepare('SELECT id, teacher_id, name FROM classes WHERE id = ? AND teacher_id = ?').get(classId, teacherId);
  }

  private assertStudentAccess(req: Request, studentId: number) {
    ...
    if (actor.role === 'parent') {
      ...
      if (!relation) throw new ApiError(403, '无权限访问该报告');
      return;
    }
  ...
  getClassOverview(req: Request, classIdInput: string) {
    ...
    if (actor.role === 'teacher' && actor.id) {
      const ownedClass = this.getClassOwnedByTeacher(classId, actor.id);
      if (!ownedClass) throw new ApiError(403, '无权限查看该班级分析');
    }
    ...
    const classInfo = db.prepare('SELECT id, name, teacher_id FROM classes WHERE id = ?').get(classId) as
      ...
      | undefined;
    if (!classInfo) throw new ApiError(404, 'Class not found');
    ...
    const topStudents = db
      ...
      .all(classId)
      .map((student: any) => ({ ...student, name: decrypt(student.name) }));
  ...
  getStudentRadar(req: Request, studentIdInput: string) {
    const studentId = Number(studentIdInput);
    if (!Number.isFinite(studentId)) throw new ApiError(400, 'Invalid studentId');
    this.assertStudentAccess(req, studentId);
