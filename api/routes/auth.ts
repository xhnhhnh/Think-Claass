  ...
  try {
    ...
    if (user) {
      if (role === 'student') {
        ...
        res.json({ 
          success: true, 
          user: { id: user.id, role: user.role, username: user.username, studentId: student?.id, name: student ? decrypt(student.name) : undefined, is_activated: !!user.is_activated },
          classFeatures: cls ? {
        ...
        res.json({ 
          success: true, 
          user: { id: user.id, parentId: user.id, role: user.role, username: user.username, studentId: student?.id, name: student ? decrypt(student.name) : undefined, is_activated: !!user.is_activated },
          classFeatures: cls ? {
            enable_chat_bubble: cls.enable_chat_bubble,
            enable_peer_review: cls.enable_peer_review,
            enable_tree_hole: cls.enable_tree_hole,
...
router.post('/activate', (req: Request, res: Response) => {
  ...
  if (!code || !userId) {
    res.status(400).json({ success: false, message: '激活码或用户ID缺失' });
    return;
  }
  ...
  try {
    ...
    if (activationCode.status === 'used') {
      res.status(400).json({ success: false, message: '该激活码已被使用' });
      return;
    }

    // Begin transaction to update code and user
    db.transaction(() => {
      db.prepare('UPDATE activation_codes SET status = ?, used_by = ?, used_at = CURRENT_TIMESTAMP WHERE id = ?')
        .run('used', userId, activationCode.id);
      db.prepare('UPDATE users SET is_activated = 1 WHERE id = ?')
        .run(userId);
    })();

    res.json({ success: true, message: '激活成功' });
  } catch (error) {
    res.status(500).json({ success: false, message: '服务器错误，激活失败' });
  }
});
