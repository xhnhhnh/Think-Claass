...
| 项 | 说明 |
| --- | --- |
| 小程序主体 | 已注册小程序并拿到 AppID（**个人主体能否开通云托管以控制台实际选项为准**：个人主体在很多能力上受限，报名与类目也另有要求，见 [00 竞赛清单](00-competition-checklist.md)） |
| 微信云托管 | 用同一个微信账号/主体开通云托管，创建（或复用）一个**环境** |
| 本地 Docker | 用来构建镜像；或者用云托管的源码构建（见步骤 1 的说明） |
...
- **推送镜像**：把镜像推到腾讯云容器镜像服务（TCR），云托管新建版本时选这个镜像。
  最常见的做法，和本仓库的 `deploy/Dockerfile` 完全兼容。
- **源码构建**：如果云托管支持从 Git 仓库构建，注意本仓库的 Dockerfile **不在根目录**，
  而在 `deploy/Dockerfile`。控制台能指定 Dockerfile 路径就用它；不能指定就先在本地构建再推 TCR。
...
| 最小副本数 | **1** | 见下方"为什么必须单副本" |
| 最大副本数 | **1** | 同上。不是省资源，是 SQLite 只能有一个写者 |
...
**为什么必须单副本（最小=最大=1）**：SQLite 是单写者数据库。两个副本同时打开同一个库文件，
会出现 `SQLITE_BUSY` / `database is locked`，严重时写丢失。这也正好解决了容器冷启动：
最小副本 1 意味着不会缩容到 0，第一个请求不会撞上冷启动。
