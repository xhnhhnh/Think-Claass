...
> **但要分清两种"备案"**：云托管免掉的是**域名备案**；**小程序备案**（工信部要求，2023-09 起新注册
> 小程序须先备案才能上架）**任何小程序都免不掉，包括这一条路**。微信公众平台首页那行「备案」就是它，
> 官方原文是「备案可与小程序代码开发并行，在小程序上架发布前完成即可」。它有自己的审核周期
> （见 [00 竞赛清单](00-competition-checklist.md) 第 1 节），是提报期里最长的一条外部依赖 —— 先提交它，
> 再来部署也不迟。

代价还有一个，是硬性的：**你必须挂一块持久化存储（CFS），并且只能跑单副本**。
