...
| 需要 ICP 备案吗 | **域名备案不需要**（流量走微信自己的通道，不经过你自己的域名）；但**小程序备案必须做** —— 2023-09 起新注册小程序须完成备案才能上架，任何路线都免不掉，入口在公众平台首页「备案」 | **两项都要**。境内主体的域名必须备案，否则无法作为服务器域名；小程序备案同样必须在发布前完成 |
...
| 成本 | 按量付费；**首个环境有 3 个月免费额度**（CPU 720 核·小时、内存 1440 GB·小时、构建 600 分钟、公网流量 5 GB），本项目 1 核 1G 常开 22 天落在额度内 ≈ 0 元；走 callContainer 不产生公网流量用量。超出后 1 核 1G 常开约 62.6 元/月（刊例价，见 10 文档） | 已有服务器时增量为 0；新购服务器按云厂商价格。没有额外平台费 |
