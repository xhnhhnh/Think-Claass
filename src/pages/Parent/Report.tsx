...
import { AlertCircle, Calendar, Heart, LoaderCircle, Star, TrendingDown, TrendingUp } from 'lucide-react';
...
function ReportNotice({
  ...
  return (
    <PageScaffold variant="detail" title="成长足迹" description={description}>
      <div className="mx-auto flex h-80 max-w-5xl flex-col items-center justify-center rounded-panel border border-warning/30 bg-surface-2 p-8 text-center shadow-raised">
        <div className="mb-6 flex size-20 items-center justify-center rounded-full bg-warning-soft">
          <Icon className={cn('size-10', iconClassName)} />
        </div>
        <h2 className="mb-3 text-2xl font-bold text-fg-1">{title}</h2>
        <p className="max-w-md text-fg-3">{description}</p>
      </div>
    </PageScaffold>
  );
}
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <DataPanel title="学习概况" surface="paper">
              <KeyValueRows
                rows={[
                  { label: '平均考试分', value: summary?.average_exam_score ?? 0, valueClassName: 'text-role' },
                  { label: '作业完成率', value: `${summary?.assignment_completion_rate ?? 0}%`, valueClassName: 'text-success' },
                  { label: '出勤率', value: `${summary?.attendance_rate ?? 0}%`, valueClassName: 'text-warning' },
                  { label: '获得表扬', value: `${summary?.praise_count ?? 0} 次`, valueClassName: 'text-danger' },
                ]}
              />
            </DataPanel>
            ...
            <DataPanel title="近期考试" surface="paper" isEmpty={recentExams.length === 0} emptyText="暂无考试记录">
              <DataList
                ...
                renderItem={(exam) => (
                  <div className="rounded-card border border-warning/10 bg-surface-2/80 p-4">
                    <div className="font-bold text-fg-1">{exam.title}</div>
                    <div className="mt-1 text-sm text-fg-3">{exam.exam_date || '未设置考试日期'}</div>
                    <div className="mt-2 text-lg font-bold text-role">
            ...
            <DataPanel title="作业与出勤" surface="paper">
              <div className="rounded-card border border-warning/10 bg-surface-2/80 p-4">
                <div className="text-sm text-fg-3">出勤明细</div>
              ...
              <DataList
                ...
                renderItem={(assignment) => (
                  <div className="rounded-card border border-warning/10 bg-surface-2/80 p-4">
                    ...
                    <div className="mt-1 text-sm text-fg-3">状态：{getAssignmentStatusText(assignment.status)}</div>
                  </div>
                )}
              />
            </DataPanel>
          ...
          <DataPanel title="红花手账" icon={Heart} iconClassName="text-role/80" surface="paper" isEmpty={records.length === 0} emptyText="还没有新的记录哦，期待宝贝的第一个闪光时刻">
            <DataList
              items={records}
              ...
              renderItem={(record) => {
                ...
                return (
                  <div className="flex items-center justify-between rounded-card border border-warning/10 bg-surface-2/80 p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-surface-2 hover:shadow-raised">
                    <div className="flex min-w-0 items-center">
                      <div className={cn('mr-5 flex size-12 shrink-0 items-center justify-center rounded-card shadow-inset', isPositive ? 'bg-success-soft text-success' : 'bg-role-soft text-role')}>
                        {isPositive ? <TrendingUp className="size-6" /> : <TrendingDown className="size-6" />}
                      </div>
                      <div className="min-w-0">
                        <p className="truncate text-[15px] font-bold text-fg-1">{record.description ?? '红花变动'}</p>
                        <p className="mt-1.5 text-xs font-medium tracking-wider text-fg-3">{formatRecordTime(record.created_at)}</p>
                      </div>
                    </div>
                    <div className={cn('flex shrink-0 items-center text-2xl font-bold', isPositive ? 'text-success' : 'text-role')}>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <DataPanel title="教师表扬与评语" surface="paper" isEmpty={praises.length === 0} emptyText="最近还没有新的表扬记录">
              <DataList
                ...
                renderItem={(praise) => (
                  <div className="rounded-card border border-warning/10 bg-surface-2/80 p-5">
                    <div className="font-bold text-fg-1">{praise.title}</div>
                    <div className="mt-2 text-fg-2">{praise.message}</div>
                    <div className="mt-3 text-xs text-fg-3">{new Date(praise.created_at).toLocaleString()}</div>
                  </div>
                )}
              />
            </DataPanel>
                ...
                renderItem={(leave) => (
                  <div className="rounded-card border border-warning/10 bg-surface-2/80 p-5">
                    <div className="flex items-center justify-between gap-4">
                      <div className="font-bold text-fg-1">{leave.reason}</div>
                    ...
                    {leave.review_comment ? <div className="mt-3 text-fg-2">{leave.review_comment}</div> : null}
                  </div>
                )}
              />
            </DataPanel>
