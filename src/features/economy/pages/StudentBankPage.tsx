...
function Sparkline({ history }: { history: string | null }) {
  ...
  if (points.length < 2) return <div className="flex h-10 items-center text-xs text-fg-3">无数据</div>;
...
export default function StudentEconomy() {
  ...
  /* The two figures rise into place; under `prefers-reduced-motion` only the fade stays. */
  const riseIn = (delay?: number) => ({
    initial: { opacity: 0, ...(shouldReduceMotion ? {} : { y: 12 }) },
    ...
    <PageScaffold variant="immersive" className="min-h-dvh px-4 pb-12 pt-16 sm:px-8">
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <h2 className="truncate text-xl font-bold tracking-tight text-fg-1">王国储蓄银行</h2>
        </div>
        ...
        <div className="flex flex-col gap-6 md:flex-row">
          ...
          <motion.div
            {...riseIn()}
            className="relative flex-1 overflow-hidden rounded-panel border border-role-ink bg-gradient-to-br from-role to-role-ink p-8 shadow-raised"
          >
            <div className="pointer-events-none absolute right-0 top-0 p-8 opacity-10">
              <Building2 className="size-48 text-role-contrast" />
            </div>

            <div className="relative z-10 mb-8">
              <div className="mb-1 flex items-center text-sm font-medium text-role-contrast/70">
                <PiggyBank className="mr-2 size-4 text-warning" />
                当前存款余额
              </div>
              <div className="flex items-baseline text-5xl font-black text-role-contrast">
              ...
              {bank && (
                <Badge
                  variant="success"
                  className="mt-3 h-auto border-success/40 bg-success/20 px-3 py-1 text-sm font-bold text-success-ink"
                >
            ...
            <div className="relative z-10 flex gap-3">
              <Button
                ...
                className="h-auto flex-1 bg-surface-2 py-3 font-bold text-role hover:bg-surface-2/90"
              >
                存入积分
              </Button>
              <Button
                onClick={() => { setBankAction('withdraw'); setShowBankModal(true); }}
                className="h-auto flex-1 border border-role-contrast/40 bg-role-contrast/10 py-3 font-bold text-role-contrast hover:bg-role-contrast/20"
              >
                提取积分
              </Button>
            </div>
          ...
          <motion.div
            {...riseIn(0.05)}
            className="relative flex-1 overflow-hidden rounded-panel border border-line-1 bg-surface-2/80 p-8 shadow-card backdrop-blur-xl"
          >
            <h3 className="mb-6 flex items-center text-2xl font-bold text-fg-1">
              <Briefcase className="mr-3 size-7 text-role" />
              我的股票资产
            </h3>

            <div className="mb-8">
              <div className="mb-1 text-sm font-medium text-fg-3">总持仓市值 (积分)</div>
              <div className="text-5xl font-black tracking-tight text-fg-1">{totalPortfolioValue}</div>
            </div>
            ...
            <div className="space-y-3">
              {portfolio.length === 0 ? (
                ...
                portfolio.map(p => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center justify-between rounded-card border border-line-1 bg-surface-3/50 p-4">
                      <div>
                        <div className="font-bold text-fg-1">
                          {p.name} <span className="ml-1 text-xs text-fg-3">{p.symbol}</span>
                        </div>
                        <div className="text-sm text-fg-3">{p.shares} 股 @ {p.average_buy_price.toFixed(1)}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-fg-1">{p.shares * p.current_price}</div>
                        <div className={`flex items-center justify-end text-xs font-bold ${isProfit ? 'text-success' : 'text-danger'}`}>
        ...
        <SectionCard
          ...
          actions={
            <span className="flex items-center font-mono text-sm text-fg-3">
            ...
            columns={[
              {
                ...
                render: (s) => (
                  <div>
                    <div className="text-lg font-bold text-fg-1">{s.name}</div>
                    <div className="font-mono text-xs text-fg-3">{s.symbol}</div>
                  </div>
                ),
              },
              {
                ...
                render: (s) => {
                  ...
                  return (
                    <div>
                      <div className={`text-2xl font-black ${isUp ? 'text-success' : 'text-danger'}`}>
                        {s.current_price}
                      </div>
                ...
                render: (s) => (
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <Button
                      ...
                      className="border border-danger/30 bg-danger/10 font-bold text-danger hover:bg-danger/20 hover:text-danger"
                    >
                      卖出
                    </Button>
                  </div>
        ...
        <Dialog open={showBankModal} onOpenChange={(open) => !open && setShowBankModal(false)}>
          <DialogContent className="sm:max-w-sm">
            <DialogHeader>
              <DialogTitle className="flex items-center text-xl font-bold text-fg-1">
            ...
            <div className="rounded-card border border-line-1 bg-surface-3/50 p-4">
              <div className="mb-1 flex justify-between text-sm">
                <span className="text-fg-3">钱包可用积分:</span>
                <span className="font-bold text-fg-1">{user?.available_points}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-fg-3">银行当前存款:</span>
                <span className="font-bold text-fg-1">{bank?.deposit_amount || 0}</span>
              </div>
            </div>
          ...
          <DialogContent className="sm:max-w-sm">
            <DialogHeader>
              <DialogTitle className="flex items-center text-xl font-bold text-fg-1">
                ...
                <span className="ml-2 rounded-card bg-surface-3/50 px-2 py-1 text-sm text-fg-2">{selectedStock?.symbol}</span>
              </DialogTitle>
            </DialogHeader>

            <div className="rounded-card border border-line-1 bg-surface-3/50 p-4">
              <div className="mb-2 flex justify-between border-b border-line-1 pb-2 text-sm">
                <span className="text-fg-3">当前单价:</span>
                <span className="font-bold text-warning">{selectedStock?.current_price} 积分</span>
              </div>
              <div className="mb-1 flex justify-between text-sm">
                <span className="text-fg-3">我的可用积分:</span>
                <span className="font-bold text-fg-1">{user?.available_points}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-fg-3">我的当前持仓:</span>
                <span className="font-bold text-fg-1">{portfolio.find(p => p.stock_id === selectedStock?.id)?.shares || 0} 股</span>
              </div>
            </div>
            ...
            <form onSubmit={handleTradeSubmit} className="space-y-4">
              ...
              {tradeShares && !isNaN(parseInt(tradeShares)) && (
                <div className="text-center text-sm text-fg-3">
                  预计交易总额: <span className="font-bold text-fg-1">{parseInt(tradeShares) * (selectedStock?.current_price ?? 0)} 积分</span>
                </div>
              )}
