...
import { familyTasksApi } from '../api/familyTasksApi';
import { engagementQueryKeys } from './useMessages';

export function useFamilyTasks(studentId: number | null) {
  return useQuery({
    queryKey: engagementQueryKeys.familyTasks(studentId),
    queryFn: async () => {
      if (!studentId) return [];
...
export function useCreateFamilyTaskMutation(studentId: number | null) {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: engagementQueryKeys.familyTasks(studentId) });
